...
# ---------------------------------------------------------------------------
# Roaming Profile API (v1)
#
# EVE AIO desktop stores ONE encrypted roaming profile per AA user here.
# The server holds only opaque ciphertext blobs and small JSON records —
# the recovery password, master key and plaintext ESI tokens never leave
# the user's PC. All endpoints are scoped to the authenticated
# EveAioServiceToken.user; object-level isolation is enforced by querying
# through that user only.
# ---------------------------------------------------------------------------

ROAMING_API_VERSION = 1
ROAMING_MAX_PAYLOAD_BYTES = 8 * 1024 * 1024   # advertised via capabilities
ROAMING_MAX_RECORD_BYTES = 256 * 1024         # heartbeats / wrapped key
ROAMING_MAX_BLOBS = 64                        # generous — client prunes to ~2
ROAMING_MAX_RECORDS = 512                     # generous — device count is tiny

import json as _json
import re as _re

from django.db import IntegrityError, transaction as _transaction
...
_ROAMING_SNAPSHOT_RE = _re.compile(r"^snapshot_\d{6,}\.roam$")
_ROAMING_RECORD_RE = _re.compile(r"^[A-Za-z0-9_.\-]+(/[A-Za-z0-9_.\-]+)*$")
_ROAMING_PREFIX_RE = _re.compile(r"^[A-Za-z0-9_.\-/]*$")


def _roaming_bad_name(name):
    return JsonResponse({"error": f"invalid object name: {str(name)[:80]}"},
                        status=400)


def _valid_record_rel(rel):
    return bool(rel) and ".." not in rel.split("/") and \
        bool(_ROAMING_RECORD_RE.match(rel))


def _roaming_profile_or_404(user):
    return EveAioRoamingProfile.objects.filter(user=user).first()


def _roaming_profile_for_write(user):
    """Fetch-or-create the user's profile row (unique per user)."""
    profile, _ = EveAioRoamingProfile.objects.get_or_create(user=user)
    return profile
...
def api_capabilities(request):
    """Capability discovery — lets EVE AIO distinguish an old plugin
    (404 on this route) from an unsupported roaming feature."""
    token_obj, err = _auth_and_get_token(request)
    if err:
        return err
    return JsonResponse({
        "roaming_profile": True,
        "roaming_api": ROAMING_API_VERSION,
        "max_payload_bytes": ROAMING_MAX_PAYLOAD_BYTES,
    })
...
@csrf_exempt
@xframe_options_exempt
def api_roaming_manifest(request):
    """Authoritative manifest — the single commit point per generation.

    PUT body: {"expected_generation": N, "manifest": {...}}
    - no committed manifest and expected == 0 -> create -> 200
    - committed and stored generation == expected -> store -> 200
    - otherwise -> 409 (stale writers never overwrite newer state)
    """
    token_obj, err = _auth_and_get_token(request)
    if err:
        return err
    ...
    if request.method == "GET":
        profile = _roaming_profile_or_404(user)
        if profile is None or not profile.manifest_json:
            return JsonResponse({"error": "no profile"}, status=404)
        return JsonResponse({"manifest": _json.loads(profile.manifest_json)})

    # PUT
    try:
        req = _json.loads(request.body or b"{}")
    except (ValueError, TypeError):
        return JsonResponse({"error": "bad json"}, status=400)
    try:
        expected = int(req.get("expected_generation"))
    except (ValueError, TypeError):
        return JsonResponse(
            {"error": "expected_generation must be an integer"}, status=400)
    man = req.get("manifest")
    if not isinstance(man, dict):
        return JsonResponse({"error": "missing manifest"}, status=400)
    man_gen = man.get("generation")
    if not isinstance(man_gen, int) or man_gen != expected + 1:
        return JsonResponse(
            {"error": "manifest generation must equal expected_generation + 1"},
            status=400)
    snap_name = man.get("snapshot_file")
    if snap_name is not None and not _ROAMING_SNAPSHOT_RE.match(str(snap_name)):
        return JsonResponse({"error": "invalid snapshot_file name"}, status=400)

    with _transaction.atomic():
        profile = (EveAioRoamingProfile.objects
                   .select_for_update()
                   .filter(user=user)
                   .first())
        committed = bool(profile and profile.manifest_json)
        cur = profile.generation if profile else 0
        if committed and expected != cur:
            return JsonResponse({"error": "stale generation",
                                 "current_generation": cur}, status=409)
        if not committed and expected != 0:
            return JsonResponse({"error": "no profile",
                                 "current_generation": cur}, status=409)
        if profile is None:
            try:
                with _transaction.atomic():
                    profile = EveAioRoamingProfile.objects.create(user=user)
            except IntegrityError:
                cur2 = (EveAioRoamingProfile.objects
                        .filter(user=user)
                        .values_list("generation", flat=True)
                        .first()) or 0
                return JsonResponse({"error": "concurrent create",
                                     "current_generation": cur2}, status=409)
        # Never promote a generation whose encrypted payload is absent —
        # the DB gives us atomicity the filesystem provider cannot.
        if snap_name is not None and not profile.roaming_objects.filter(
                kind=ROAMING_KIND_BLOB, name=str(snap_name)).exists():
            return JsonResponse({"error": "referenced snapshot missing",
                                 "current_generation": cur}, status=409)
        # Single-statement conditional update — atomic on every backend,
        # so two competing writers can never both promote a generation.
        updated = (EveAioRoamingProfile.objects
                   .filter(pk=profile.pk, generation=cur)
                   .update(manifest_json=_json.dumps(man),
                           generation=man_gen,
                           profile_id=str(man.get("profile_id") or "")[:64],
                           updated_at=timezone.now()))
        if not updated:
            fresh = (EveAioRoamingProfile.objects
                     .filter(pk=profile.pk)
                     .values_list("generation", flat=True)
                     .first()) or 0
            return JsonResponse({"error": "stale generation",
                                 "current_generation": fresh}, status=409)

    logger.info("EVE AIO roaming: %s committed generation %s (%s)",
                user, man_gen, profile.profile_id or "no-id")
    return JsonResponse({"ok": True, "generation": man_gen})


@require_GET
@csrf_exempt
@xframe_options_exempt
def api_roaming_blobs(request):
    """List snapshot names stored for the user."""
    token_obj, err = _auth_and_get_token(request)
    if err:
        return err
    profile = _roaming_profile_or_404(token_obj.user)
    names = []
    if profile is not None:
        names = sorted(profile.roaming_objects
                       .filter(kind=ROAMING_KIND_BLOB)
...
@csrf_exempt
@xframe_options_exempt
def api_roaming_blob(request, name):
    """Encrypted snapshot blob storage — opaque bytes in/out."""
    token_obj, err = _auth_and_get_token(request)
    if err:
        return err
    if not _ROAMING_SNAPSHOT_RE.match(name or ""):
        return _roaming_bad_name(name)
    user = token_obj.user

    if request.method == "GET":
        obj = (EveAioRoamingObject.objects
               .filter(profile__user=user, kind=ROAMING_KIND_BLOB, name=name)
               .first())
        if obj is None:
            return JsonResponse({"error": "no blob"}, status=404)
        return HttpResponse(bytes(obj.payload or b""),
                            content_type="application/octet-stream")

    if request.method == "DELETE":
        (EveAioRoamingObject.objects
         .filter(profile__user=user, kind=ROAMING_KIND_BLOB, name=name)
         .delete())
        return HttpResponse(status=204)

    # PUT — raw octet-stream body
    try:
        cl = request.META.get("CONTENT_LENGTH")
        if cl is not None and int(cl) > ROAMING_MAX_PAYLOAD_BYTES:
            return JsonResponse({"error": "payload too large"}, status=413)
    except (ValueError, TypeError):
        pass
    body = request.read(ROAMING_MAX_PAYLOAD_BYTES + 1) or b""
    if len(body) > ROAMING_MAX_PAYLOAD_BYTES:
        return JsonResponse({"error": "payload too large"}, status=413)
    profile = _roaming_profile_for_write(user)
    existing = profile.roaming_objects.filter(kind=ROAMING_KIND_BLOB)
    if not existing.filter(name=name).exists() and \
            existing.count() >= ROAMING_MAX_BLOBS:
        return JsonResponse({"error": "too many blobs"}, status=413)
    profile.roaming_objects.update_or_create(
        kind=ROAMING_KIND_BLOB,
        name=name,
        defaults={"payload": body, "size": len(body), "record_json": ""},
    )
    logger.info("EVE AIO roaming: %s put blob %s (%d bytes)",
                user, name, len(body))
...
@csrf_exempt
@xframe_options_exempt
def api_roaming_records(request):
    """List JSON records, optionally filtered by ?prefix=devices/."""
    token_obj, err = _auth_and_get_token(request)
    if err:
        return err
    prefix = request.GET.get("prefix", "")
    if ".." in prefix or not _ROAMING_PREFIX_RE.match(prefix):
        return JsonResponse({"error": "invalid prefix"}, status=400)
    profile = _roaming_profile_or_404(token_obj.user)
    records = {}
    if profile is not None:
        qs = profile.roaming_objects.filter(kind=ROAMING_KIND_RECORD)
        if prefix:
            qs = qs.filter(name__startswith=prefix)
        for obj in qs:
            try:
                records[obj.name] = _json.loads(obj.record_json)
            except (ValueError, TypeError):
                continue
...
@csrf_exempt
@xframe_options_exempt
def api_roaming_record(request, rel):
    """Small JSON records: recovery.json, devices/<id>.json, ..."""
    token_obj, err = _auth_and_get_token(request)
    if err:
        return err
    if not _valid_record_rel(rel):
        return _roaming_bad_name(rel)
    user = token_obj.user

    if request.method == "GET":
        obj = (EveAioRoamingObject.objects
               .filter(profile__user=user, kind=ROAMING_KIND_RECORD, name=rel)
               .first())
        if obj is None:
            return JsonResponse({"error": "no record"}, status=404)
        return JsonResponse({"record": _json.loads(obj.record_json)})
    ...
    if not isinstance(rec, dict):
        return JsonResponse({"error": "missing record"}, status=400)
    encoded = _json.dumps(rec)
    if len(encoded.encode("utf-8")) > ROAMING_MAX_RECORD_BYTES:
        return JsonResponse({"error": "payload too large"}, status=413)
    profile = _roaming_profile_for_write(user)
    existing = profile.roaming_objects.filter(kind=ROAMING_KIND_RECORD)
    if not existing.filter(name=rel).exists() and \
            existing.count() >= ROAMING_MAX_RECORDS:
        return JsonResponse({"error": "too many records"}, status=413)
    profile.roaming_objects.update_or_create(
        kind=ROAMING_KIND_RECORD,
        name=rel,
        defaults={"record_json": encoded, "payload": None,
